...
void UVioPropagator::propagate(std::shared_ptr<UVioState> state, double timestamp) {
  ...
  if(state->_state->_timestamp > timestamp) {
    ...
    if(prop_data.size() > 1) last_w = prop_data.at(prop_data.size()-2).wm - state->_state->_imu->bias_g();
    else if(!prop_data.empty()) last_w = prop_data.at(prop_data.size()-1).wm - state->_state->_imu->bias_g();
